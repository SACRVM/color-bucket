...
(function (root, factory) {
    ...
    /**
     * A saved palette as CSS custom properties.
     *
     * The sibling of toCSS, for the colours a person actually kept rather than
     * the ones this file generated. An entry that came from a generated palette
     * carries the label it had there — "neutral 500", "Cadmium Yellow +
     * Ultramarine Blue" — and that label is what names the property, because a
     * ramp step called --neutral-500 is a token and one called --color-7 is a
     * number. Anything hand-mixed has no name to offer and gets counted.
     *
     * @param {Array<{hex:string,label?:string}>} entries
     */
    function paletteToCSS(entries) {
        const list = Array.isArray(entries) ? entries.filter((e) => e && e.hex) : [];
        if (!list.length) return "";
        const lines = ["/* Color Bucket — " + list.length
            + (list.length === 1 ? " colour */" : " colours */")];
        const used = Object.create(null);
        let plain = 0;
        for (const e of list) {
            const base = e.label ? slug(e.label) : "color-" + (++plain);
            used[base] = (used[base] || 0) + 1;
            const name = used[base] > 1 ? base + "-" + used[base] : base;
            lines.push("--" + name + ": " + up(e.hex) + ";");
        }
        return lines.join("\n");
    }
